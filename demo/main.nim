...
proc test_window(ctx: PContext) =
  ...
  if ctx.begin_window("demo win", mu.rect(40,40,300,450)):
    ...
    # lable + buttons
    if ctx.header_ex("test buttons", OPT_EXPANDED):
      ctx.layout_row(3, [86.cint, -110, -1], 0)
      ctx.label("test btn1:")
      if ctx.button("btn1:"): wlog("pres btn1")
      if ctx.button("btn2:"): wlog("pres btn2")
      ctx.label("test btn2:")
      if ctx.button("btn3:"): wlog("pres btn3")
    ...
    # tree
    if ctx.header_ex("tree and text", OPT_EXPANDED):
      ...
      if ctx.begin_treenode("test 1"):
        ...
        if ctx.begin_treenode("test 1b"):
          if ctx.button("tbtn1:"): wlog("pres tbtn1")
          if ctx.button("tbtn2:"): wlog("pres tbtn2")
          ctx.end_treenode()
        ctx.end_treenode()

      if ctx.begin_treenode("test 2"):
        ctx.layout_row(2, [54, 54], 0)
        if ctx.button("tbtn3:"): wlog("pres tbtn3")
        if ctx.button("tbtn4:"): wlog("pres tbtn4")
        if ctx.button("tbtn5:"): wlog("pres tbtn5")
        if ctx.button("tbtn6:"): wlog("pres tbtn6")
        ctx.end_treenode()
    ...
    if ctx.header_ex("bg color", OPT_EXPANDED):
...
proc log_window(ctx: PContext) =
  if ctx.begin_window("log win", mu.rect(350, 40, 300, 200)):
    # output text panel
...
proc style_window(ctx: PContext) =
  ...
  if ctx.begin_window("style editor", mu.rect(350, 250, 300, 240)):
    var
...
proc main =
  ...
  while bRun:
    ...
    while ctx.next_command(cmd.addr):
      case cmd.typec:
